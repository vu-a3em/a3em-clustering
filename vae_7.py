...
if __name__ == '__main__':
    ...
    with open(f'{args.output}/loss.csv', 'w') as log:
        ...
        for i in range(256):
            ...
            log.flush()
            if (i + 1) % 8 == 0:
                torch.save(encoder.state_dict(), f'{args.output}/encoder-F{args.embedding_size}-E{i+1}-L{round(eval_loss)}.pt')
                torch.save(decoder.state_dict(), f'{args.output}/decoder-F{args.embedding_size}-E{i+1}-L{round(eval_loss)}.pt')
