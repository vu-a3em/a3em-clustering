...
class Decoder(torch.nn.Module):
    def __init__(self, *, embedding_size):
        ...
        self.convolutions = torch.nn.ModuleList([
            torch.nn.ConvTranspose2d(256, 128, kernel_size = (3, 3), stride = 2),
            ...
            torch.nn.ConvTranspose2d(64, 64, kernel_size = (3, 3), stride = 2, output_padding = 1),
            torch.nn.Conv2d(64, 64, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(64, 64, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(64, 32, kernel_size = (3, 3), stride = 2),
    ...
    def forward(self, x):
        ...
        for i, step in enumerate(self.convolutions):
            if i == len(self.convolutions) - 1:
                return step(x).reshape(x.shape[0], 65, 65)
            x = relu(step(x))
            # print(f'< {x.shape}')
