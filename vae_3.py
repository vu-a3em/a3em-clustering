...
class Decoder(torch.nn.Module):
    def __init__(self, *, embedding_size):
        ...
        self.convolutions = torch.nn.ModuleList([
            torch.nn.ModuleList([
                torch.nn.ConvTranspose2d(64, 128, kernel_size = (4, 4), stride = 2, output_padding = (0, 1)),
                torch.nn.Conv2d(128, 128, kernel_size = (7, 7), padding = 3, padding_mode = PADDING_MODE),
                torch.nn.Conv2d(128, 128, kernel_size = (5, 5), padding = 2, padding_mode = PADDING_MODE),
                torch.nn.Conv2d(128, 128, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            ]),
            torch.nn.ModuleList([
                torch.nn.ConvTranspose2d(128, 256, kernel_size = (4, 4), stride = 2, output_padding = (0, 0)),
                torch.nn.Conv2d(256, 256, kernel_size = (7, 7), padding = 3, padding_mode = PADDING_MODE),
                torch.nn.Conv2d(256, 256, kernel_size = (5, 5), padding = 2, padding_mode = PADDING_MODE),
                torch.nn.Conv2d(256, 256, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            ]),
            torch.nn.ModuleList([
                torch.nn.ConvTranspose2d(256, 128, kernel_size = (4, 4), stride = 2, output_padding = (1, 1)),
                torch.nn.Conv2d(128, 128, kernel_size = (7, 7), padding = 3, padding_mode = PADDING_MODE),
                torch.nn.Conv2d(128, 128, kernel_size = (5, 5), padding = 2, padding_mode = PADDING_MODE),
                torch.nn.Conv2d(128, 128, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            ]),
            torch.nn.ModuleList([
                ...
                torch.nn.Conv2d(1, 1, kernel_size = (7, 7), padding = 3, padding_mode = PADDING_MODE),
                torch.nn.Conv2d(1, 1, kernel_size = (5, 5), padding = 2, padding_mode = PADDING_MODE),
                torch.nn.Conv2d(1, 1, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            ]),
        ])
    def forward(self, x):
        ...
        for i, steps in enumerate(self.convolutions):
            for j, step in enumerate(steps):
                if i == len(self.convolutions) - 1 and j == len(steps) - 1:
