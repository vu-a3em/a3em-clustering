...
class Encoder(torch.nn.Module):
    def __init__(self, *, embedding_size):
        ...
        self.convolutions = torch.nn.ModuleList([
            torch.nn.Conv2d(1, 32, kernel_size = (3, 3), stride = (3, 1)),
            ...
            torch.nn.Conv2d(32, 64, kernel_size = (2, 2), stride = 2),
            torch.nn.Conv2d(64, 128, kernel_size = (1, 1)),
            torch.nn.Conv2d(128, 64, kernel_size = (1, 1)),
            torch.nn.Conv2d(64, 32, kernel_size = (1, 1)),

            torch.nn.Conv2d(32, 64, kernel_size = (2, 2)),
            torch.nn.Conv2d(64, 128, kernel_size = (1, 1)),
            torch.nn.Conv2d(128, 64, kernel_size = (1, 1)),
            torch.nn.Conv2d(64, 32, kernel_size = (1, 1)),
        ])
...
class Decoder(torch.nn.Module):
    def __init__(self, *, embedding_size):
        ...
        self.convolutions = torch.nn.ModuleList([
            torch.nn.ConvTranspose2d(64, 128, kernel_size = (2, 2)),
            torch.nn.Conv2d(128, 128, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(128, 128, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(128, 128, kernel_size = (2, 2), stride = 2, output_padding = (0, 1)),
            torch.nn.Conv2d(128, 128, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(128, 128, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(128, 64, kernel_size = (3, 3), stride = 2, output_padding = (0, 1)),
            ...
            torch.nn.ConvTranspose2d(64, 32, kernel_size = (5, 5), stride = (3, 1), padding = 1),
...
if __name__ == '__main__':
    ...
    s = dataloader.get_spectrogram(next(iter(dataloader.get_dataset(1, 1).items()))[1][0])[-1].shape
    ...
    raw_dataset = dataloader.get_dataset(128, 8192)
    ...
    def loss_fn(before, after, mean, logstd):
        ...
        kldiv_loss = torch.mean(-0.5 * torch.sum(1 + 2 * logstd - logstd.exp()**2 - mean**2, axis = 1))
    ...
    def do_training_epoch(epoch):
        ...
        for i in range(k):
            ...
            if (i + 1) % batches_per_opt_step == 0:
                opt.step()
            print(f'\repoch {epoch}: {i+1}/{k} complete ({100*(i+1)/k:.2f}%) ... batch loss {loss.item():.2f} (mse {mse_loss.item():.2f}) (kldiv {kldiv_loss.item():.2f})          ', end = '')
        res = np.mean(losses, axis = 0)
        print(f'\repoch {epoch}: avg loss {res[0]:.4f} (mse {res[1]:.4f}) (kldiv {res[2]:.4f})                                                                                     ')
        return res
    def do_eval_epoch():
        encoder.eval()
        decoder.eval()
