...
class Encoder(torch.nn.Module):
    def __init__(self, *, embedding_size):
        ...
        self.convolutions = torch.nn.ModuleList([
            torch.nn.Conv2d(1, 8, kernel_size = (3, 3), stride = (1, 2)),
            torch.nn.Conv2d(8, 16, kernel_size = (1, 1)),
            torch.nn.Conv2d(16, 8, kernel_size = (1, 1)),
            torch.nn.Conv2d(8, 4, kernel_size = (1, 1)),

            torch.nn.Conv2d(4, 8, kernel_size = (3, 3), stride = (1, 2)),
            torch.nn.Conv2d(8, 16, kernel_size = (1, 1)),
            torch.nn.Conv2d(16, 8, kernel_size = (1, 1)),
            torch.nn.Conv2d(8, 4, kernel_size = (1, 1)),

            torch.nn.Conv2d(4, 8, kernel_size = (3, 3), stride = 2),
            torch.nn.Conv2d(8, 16, kernel_size = (1, 1)),
            torch.nn.Conv2d(16, 8, kernel_size = (1, 1)),
            torch.nn.Conv2d(8, 4, kernel_size = (1, 1)),
        ...
        self.mean = torch.nn.Linear(24, embedding_size)
        self.logstd = torch.nn.Linear(24, embedding_size)
    def forward(self, x):
        x = self.quant(x)
...
class Decoder(torch.nn.Module):
    def __init__(self, *, embedding_size):
        ...
        self.linears = torch.nn.ModuleList([
            torch.nn.Linear(embedding_size, 24),
        ])
        self.convolutions = torch.nn.ModuleList([
            torch.nn.ConvTranspose2d(24, 16, kernel_size = (2, 3), stride = 2),
            torch.nn.Conv2d(16, 16, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(16, 16, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(16, 16, kernel_size = (3, 3), stride = 2),
            torch.nn.Conv2d(16, 16, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(16, 16, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(16, 8, kernel_size = (3, 3), stride = 2, output_padding=(1, 0)),
            torch.nn.Conv2d(8, 8, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(8, 8, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(8, 8, kernel_size = (3, 3), stride = (1, 2), output_padding=(0, 1)),
            torch.nn.Conv2d(8, 8, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(8, 8, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(8, 4, kernel_size = (3, 3), stride = (1, 2)),
            torch.nn.Conv2d(4, 4, kernel_size = (5, 5), padding = 2, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(4, 1, kernel_size = (1, 1)),
        ])
    def forward(self, x):
...
if __name__ == '__main__':
    # import ptflops
    # macs, parms = ptflops.get_model_complexity_info(Encoder(embedding_size = 16), (16, 65))
    # print(macs, parms)
