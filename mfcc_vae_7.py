...
class Decoder(torch.nn.Module):
    def __init__(self, *, embedding_size):
        ...
        self.linears = torch.nn.ModuleList([
            torch.nn.Linear(embedding_size, 32),
            torch.nn.Linear(32, 64),
        ])
        self.convolutions = torch.nn.ModuleList([
            torch.nn.ConvTranspose2d(64, 32, kernel_size = (2, 3), stride = 2),
            torch.nn.Conv2d(32, 32, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(32, 32, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(32, 32, kernel_size = (3, 3), stride = 2),
            torch.nn.Conv2d(32, 32, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(32, 32, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(32, 16, kernel_size = (3, 3), stride = 2, output_padding=(1, 0)),
            torch.nn.Conv2d(16, 16, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(16, 16, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(16, 16, kernel_size = (3, 3), stride = (1, 2), output_padding=(0, 1)),
            torch.nn.Conv2d(16, 16, kernel_size = (3, 3), padding = 1, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(16, 16, kernel_size = (1, 1)),

            torch.nn.ConvTranspose2d(16, 8, kernel_size = (3, 3), stride = (1, 2)),
            torch.nn.Conv2d(8, 8, kernel_size = (5, 5), padding = 2, padding_mode = PADDING_MODE),
            torch.nn.Conv2d(8, 1, kernel_size = (1, 1)),
        ])
    def forward(self, x):
